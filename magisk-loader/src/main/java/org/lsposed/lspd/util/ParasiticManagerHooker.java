...
public class ParasiticManagerHooker {
    ...
    private static void sendBinderToManager(final ClassLoader classLoader, IBinder binder) {
        try {
            var clazz = FposedHelpers.findClass("org.lsposed.manager.Constants", classLoader);
            var ok = (boolean) FposedHelpers.callStaticMethod(clazz, "setBinder",
                    new Class[]{IBinder.class}, binder);
            if (ok) return;
            throw new RuntimeException("setBinder: " + false);
        } catch (Throwable t) {
            Utils.logW("Could not send binder to LFPosed Manager", t);
        }
    }
    ...
    private static void hookForManager(ILFPManagerService managerService) {
        var managerApkHooker = new FC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) {
                ...
                ApplicationInfo appInfo = (ApplicationInfo) FposedHelpers.getObjectField(bindData, "appInfo");
                FposedHelpers.setObjectField(bindData, "appInfo", getManagerPkgInfo(appInfo).applicationInfo);
            }
        };
        FposedHelpers.findAndHookMethod(ActivityThread.class,
                "handleBindApplication",
                "android.app.ActivityThread$AppBindData",
                managerApkHooker);

        var unhooks = new FC_MethodHook.Unhook[]{null};
        unhooks[0] = FposedHelpers.findAndHookMethod(
                LoadedApk.class, "getClassLoader", new FC_MethodHook() {
                    @Override
                    protected void afterHookedMethod(MethodHookParam param) {
                        var pkgInfo = getManagerPkgInfo(null);
                        if (pkgInfo != null && FposedHelpers.getObjectField(param.thisObject, "mApplicationInfo") == pkgInfo.applicationInfo) {
                            ...
                            var pathList = FposedHelpers.getObjectField(pathClassLoader, "pathList");
                            List<String> lstDexPath = (List<String>)FposedHelpers.callMethod(pathList, "getDexPaths");
                            if (!lstDexPath.contains(sSourceDir)) {
                                Utils.logW("Could not find manager apk injected in classloader");
                                FposedHelpers.callMethod(pathClassLoader, "addDexPath", sSourceDir);
                            }
        ...
        var activityClientRecordClass = FposedHelpers.findClass("android.app.ActivityThread$ActivityClientRecord", ActivityThread.class.getClassLoader());
        var activityHooker = new FC_MethodHook() {
            @Override
            protected void beforeHookedMethod(MethodHookParam param) {
            ...
            protected void afterHookedMethod(MethodHookParam param) {
                for (var i = 0; i < param.args.length && activityClientRecordClass.isInstance(param.thisObject); ++i) {
                    if (param.args[i] instanceof ActivityInfo) {
                        ...
                        Hookers.logD("loading state of " + aInfo.name);
                        states.computeIfPresent(aInfo.name, (k, v) -> {
                            FposedHelpers.setObjectField(param.thisObject, "state", v);
                            return v;
                        });
                        persistentStates.computeIfPresent(aInfo.name, (k, v) -> {
                            FposedHelpers.setObjectField(param.thisObject, "persistentState", v);
                            return v;
                        });
                    }
                }
        ...
        FposedBridge.hookAllConstructors(activityClientRecordClass, activityHooker);

        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.O_MR1) {
            FposedBridge.hookAllMethods(FposedHelpers.findClass("android.app.ActivityThread$ApplicationThread", ActivityThread.class.getClassLoader()), "scheduleLaunchActivity", activityHooker);
        }

        FposedBridge.hookAllMethods(ActivityThread.class, "handleReceiver", new FC_MethodReplacement() {
            @Override
        ...
        FposedBridge.hookAllMethods(ActivityThread.class, "installProvider", new FC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) {
                ...
                if (ctx != null && info != null && pkgInfo != null) {
                    ...
                    if (originalContext == null) {
                        ...
                        FposedHelpers.setObjectField(originalPkgInfo, "mPackageName", packageName);
                        originalContext = (Context) FposedHelpers.callStaticMethod(FposedHelpers.findClass("android.app.ContextImpl", null),
        ...
        FposedHelpers.findAndHookMethod(ActivityThread.class, "deliverNewIntents", activityClientRecordClass, List.class, new FC_MethodHook() {
            @Override
            protected void beforeHookedMethod(MethodHookParam param) {
        ...
        FposedHelpers.findAndHookMethod(WebViewFactory.class, "getProvider", new FC_MethodReplacement() {
            @Override
            protected Object replaceHookedMethod(MethodHookParam param) {
                var sProviderInstance = FposedHelpers.getStaticObjectField(WebViewFactory.class, "sProviderInstance");
                if (sProviderInstance != null) return sProviderInstance;
                //noinspection unchecked
                var providerClass = (Class<WebViewFactoryProvider>) FposedHelpers.callStaticMethod(WebViewFactory.class, "getProviderClass");
                ...
                try {
                    ...
                    FposedHelpers.setStaticObjectField(WebViewFactory.class, "sProviderInstance", sProviderInstance);
        ...
        var stateHooker = new FC_MethodHook() {
            @Override
            protected void beforeHookedMethod(MethodHookParam param) {
                try {
                    var record = param.args[0];
                    if (record instanceof IBinder) {
                        record = ((ArrayMap<?, ?>) FposedHelpers.getObjectField(param.thisObject, "mActivities")).get(record);
                        if (record == null) return;
                    }
                    FposedHelpers.callMethod(param.thisObject, Build.VERSION.SDK_INT >= Build.VERSION_CODES.P ? "callActivityOnSaveInstanceState" : "callCallActivityOnSaveInstanceState", record);
                    var state = (Bundle) FposedHelpers.getObjectField(record, "state");
                    var persistentState = (PersistableBundle) FposedHelpers.getObjectField(record, "persistentState");
                    var aInfo = (ActivityInfo) FposedHelpers.getObjectField(record, "activityInfo");
        ...
        FposedBridge.hookAllMethods(ActivityThread.class, "performStopActivityInner", stateHooker);
        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.O_MR1)
